...
def test_knight_blocked():
    ...
    for enemy_position in enemy_positions:
        ...
        for blocking_position in path_1_blocking_positions + path_2_blocking_positions:
            curr_game_state = copy.deepcopy(empty_game)
            curr_game_state["board_state"][3][3] = [{"type": f"white_knight"}]
            curr_game_state["board_state"][enemy_position[0]][enemy_position[1]] = [{"type": f"black pawn"}]
            curr_game_state["board_state"][blocking_position[0]][blocking_position[1]] = [{"type": f"black_pawn"}]
        ...
        for path_1_blocking_index in range(2):
            for path_2_blocking_index in range(2):
                ...
                for enemy_position_ in enemy_positions:
                    curr_game_state["board_state"][enemy_position_[0]][enemy_position_[1]] = [{"type": f"black pawn"}]

                curr_game_state["board_state"][path_1_blocking_position[0]][path_1_blocking_position[1]] = [{"type": f"black_pawn"}]
                curr_game_state["board_state"][path_2_blocking_position[0]][path_2_blocking_position[1]] = [{"type": f"black_pawn"}]
