...
**Description:** League of Chess is an online revision of the classic game of Chess that reimagines the traditional game with MOBA-style mechanics inspired by League of Legends. This unique chess variant introduces dynamic gameplay through modified piece mechanics, neutral objectives (monsters), and strategic buff systems that dramatically alter the flow of the game.

**Inspiration Source:** The concept originated from a [Reddit thread](https://www.reddit.com/r/leagueoflegends/comments/3yf3d3/chess_patch_11_notes_by_riot_games/) posted by /u/Zaaptastic on /r/leagueoflegends, which parodied video game balance patches. The rules were so intriguing that they were brought to life as a fully playable game.

**Purpose:** To create an engaging chess variant that combines the strategic depth of chess with the dynamic, objective-based gameplay of MOBAs. The game aims to quicken the pace of traditional chess while introducing new strategic layers through neutral objectives, piece buffs, and evolving piece mechanics.

**Current Development Status:** This project is actively in development (WORK IN PROGRESS).

- **Frontend:** ~75% complete - UI components for board, pieces, neutral monsters, status effects, and win/loss screens are implemented. Remaining work includes neutral monster buff UI enhancements, shop rework, pawn exchange, and various visual cues.
- **Backend:** ~80% complete - Core game logic, move generation, check/checkmate detection, and API endpoints are functional. Active work focuses on finalizing neutral monster buff implementation and improving code maintainability.
- **Production:** ~30% complete - Project is Dockerized with basic deployment structure. Remaining work includes database setup documentation, code refactoring, linting, and Kubernetes deployment.

For detailed patch notes, game rules, and a comprehensive roadmap, see [README.md](README.md).
...
| MongoDB | 4.3.3 | NoSQL database for game state persistence (via PyMongo) |
...
| Testing Library | ^13.4.0 | Component testing utilities |

### Deployment & Infrastructure

| Technology | Purpose |
|------------|---------|
| Docker | Containerization (multi-stage build with Node + Nginx + Python) |
| Nginx | Static file server for frontend assets |
| node:current-alpine | Frontend build stage base image |
| nginx:alpine | Production runtime base image |

**Key Dependencies:**
- Backend dependencies are managed in `backend/requirements.txt`
- Frontend dependencies are managed in `frontend/package.json`
- Python virtual environment is used in Docker for PEP 668 compliance
